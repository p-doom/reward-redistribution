...
def make_gymnax_env(env_name: str, config: DictConfig) -> Tuple[Environment, Environment]:
    ...
    env_params_kwargs = config.env.kwargs
    if env_params_kwargs:
        env_params = dataclasses.replace(env_params, **env_params_kwargs)
        eval_env_params = dataclasses.replace(eval_env_params, **env_params_kwargs)
